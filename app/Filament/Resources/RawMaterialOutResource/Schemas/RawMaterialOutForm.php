...
namespace App\Filament\Resources\RawMaterialOutResource\Schemas;
...
final class RawMaterialOutForm
{
        ...
        return $schema
            ->components([
                Section::make('معلومات الإخراج')
                    ->schema([
                        ...
                        Select::make('consumer_id')
                            ...
                            ->disabled(
                                fn (string $operation, ?RawMaterialOut $record) => $operation === 'edit' && $record?->status === MaterialEntranceStatus::Closed
                            ),
                        ...
                        Placeholder::make('status_display')
                            ->label('الحالة')
                            ->content(fn (?RawMaterialOut $record) => $record?->status?->getLabel() ?? MaterialEntranceStatus::Draft->getLabel()),

                        Placeholder::make('closed_at_display')
                            ->label('تاريخ الإغلاق')
                            ->content(fn (?RawMaterialOut $record) => $record?->closed_at?->format('Y-m-d H:i') ?? '-')
                            ->visible(fn (?RawMaterialOut $record) => $record?->status === MaterialEntranceStatus::Closed),
                ...
                Section::make('الأصناف')
                    ->schema([
                        ProductSelector::make('product_selector')
                            ->disabled(
                                fn (string $operation, ?RawMaterialOut $record) => $operation === 'edit' && $record?->status === MaterialEntranceStatus::Closed
                            ),
