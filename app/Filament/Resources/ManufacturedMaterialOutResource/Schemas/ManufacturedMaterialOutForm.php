...
namespace App\Filament\Resources\ManufacturedMaterialOutResource\Schemas;
...
final class ManufacturedMaterialOutForm
{
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->components([
                Section::make('معلومات الإخراج')
                    ->schema([
                        Hidden::make('user_id')
                            ->default(fn () => Auth::id()),
                        ...
                        Select::make('consumer_id')
                            ->label('المستهلك')
                            ->relationship(name: 'consumer', titleAttribute: 'name')
                            ->required()
                            ->searchable()
                            ->preload()
                            ->createOptionForm([
                            ...
                            ->disabled(
                                fn (string $operation, ?ManufacturedMaterialOut $record) => $operation === 'edit' && $record?->status === MaterialEntranceStatus::Closed
                            ),
                        ...
                        Placeholder::make('status_display')
                            ->label('الحالة')
                            ->content(fn (?ManufacturedMaterialOut $record) => $record?->status?->getLabel() ?? MaterialEntranceStatus::Draft->getLabel()),

                        Placeholder::make('closed_at_display')
                            ->label('تاريخ الإغلاق')
                            ->content(fn (?ManufacturedMaterialOut $record) => $record?->closed_at?->format('Y-m-d H:i') ?? '-')
                            ->visible(fn (?ManufacturedMaterialOut $record) => $record?->status === MaterialEntranceStatus::Closed),
