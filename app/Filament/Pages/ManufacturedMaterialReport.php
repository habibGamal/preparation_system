...
use App\Filament\Exports\ManufacturedMaterialReportExporter;
 ...
 */
final class ManufacturedMaterialReport extends Page implements HasTable
{
    ...
    protected static ?string $navigationLabel = 'تقرير المصنعات';

    protected static ?string $title = 'تقرير حركة المصنعات';

    protected string $view = 'filament.pages.manufactured-material-report';
    ...
    public function table(Table $table): Table
    {
        ...
        return $table
            ...
            ->headerActions([
                ExportAction::make()
                    ->exporter(ManufacturedMaterialReportExporter::class)
                    ->label('تصدير')
                    ->color('primary'),
            ])
            ->defaultSort('name');
    ...
    private function buildProductReportQuery(Carbon $startAt, Carbon $endAt)
    {
        ...
        $inletBeforeSub = ManufacturedMaterialEntranceItem::query()
            ->selectRaw('manufactured_material_entrance_items.product_id, COALESCE(SUM(manufactured_material_entrance_items.quantity), 0) as quantity_total')
            ->join('manufactured_material_entrances', 'manufactured_material_entrances.id', '=', 'manufactured_material_entrance_items.manufactured_material_entrance_id')
            ->where('manufactured_material_entrances.status', MaterialEntranceStatus::Closed->value)
            ->whereNotNull('manufactured_material_entrances.closed_at')
            ->where('manufactured_material_entrances.closed_at', '<', $startDateTime)
            ->groupBy('manufactured_material_entrance_items.product_id');

        $inletPeriodSub = ManufacturedMaterialEntranceItem::query()
            ->selectRaw('manufactured_material_entrance_items.product_id, COALESCE(SUM(manufactured_material_entrance_items.quantity), 0) as quantity_total')
            ->join('manufactured_material_entrances', 'manufactured_material_entrances.id', '=', 'manufactured_material_entrance_items.manufactured_material_entrance_id')
            ->where('manufactured_material_entrances.status', MaterialEntranceStatus::Closed->value)
            ->whereNotNull('manufactured_material_entrances.closed_at')
            ->whereBetween('manufactured_material_entrances.closed_at', [$startDateTime, $endDateTime])
            ->groupBy('manufactured_material_entrance_items.product_id');

        $outletBeforeSub = ManufacturedMaterialOutItem::query()
            ->selectRaw('manufactured_material_out_items.product_id, COALESCE(SUM(manufactured_material_out_items.quantity), 0) as quantity_total')
            ->join('manufactured_material_outs', 'manufactured_material_outs.id', '=', 'manufactured_material_out_items.manufactured_material_out_id')
            ->where('manufactured_material_outs.status', MaterialEntranceStatus::Closed->value)
            ->whereNotNull('manufactured_material_outs.closed_at')
            ->where('manufactured_material_outs.closed_at', '<', $startDateTime)
            ->groupBy('manufactured_material_out_items.product_id');

        $outletPeriodSub = ManufacturedMaterialOutItem::query()
            ->selectRaw('manufactured_material_out_items.product_id, COALESCE(SUM(manufactured_material_out_items.quantity), 0) as quantity_total')
            ->join('manufactured_material_outs', 'manufactured_material_outs.id', '=', 'manufactured_material_out_items.manufactured_material_out_id')
            ->where('manufactured_material_outs.status', MaterialEntranceStatus::Closed->value)
            ->whereNotNull('manufactured_material_outs.closed_at')
            ->whereBetween('manufactured_material_outs.closed_at', [$startDateTime, $endDateTime])
            ->groupBy('manufactured_material_out_items.product_id');
        ...
        $stocktakingBeforeSub = StocktakingItem::query()
            ...
            ->where('stocktakings.product_type', ProductType::Manufactured->value)
        ...
        $wasteBeforeSub = WastedItem::query()
            ...
            ->where('wastes.type', ProductType::Manufactured->value)
