...
use App\Filament\Exports\ManufacturedSupplierEntranceReportExporter;
use App\Filament\Exports\RawSupplierEntranceReportExporter;
 ...
 */
final class SupplierEntranceReport extends Page implements HasTable
{
    ...
    protected static ?string $navigationLabel = 'تقرير إدخالات الموردين';

    protected static ?string $title = 'تقرير إدخالات مورد';

    protected string $view = 'filament.pages.supplier-entrance-report';
    ...
    public function table(Table $table): Table
    {
        ...
        $reportQuery = $this->buildSupplierItemsQuery();
        ...
        return $table
            ...
            ->headerActions([
                ExportAction::make()
                    ->exporter($this->activeTab === 'manufactured' ? ManufacturedSupplierEntranceReportExporter::class : RawSupplierEntranceReportExporter::class)
                    ->label('تصدير')
                    ->color('primary'),
            ])
    ...
    private function buildSupplierItemsQuery()
    {
        [$startAt, $endAt] = $this->resolvePeriod();
        $supplierId = (int) ($this->data['supplier_id'] ?? 0);

        if ($this->activeTab === 'manufactured') {
            $itemTable = 'manufactured_material_entrance_items';
            $entranceTable = 'manufactured_material_entrances';
            $entranceForeignKey = 'manufactured_material_entrance_id';
            $productType = ProductType::Manufactured->value;
            $query = ManufacturedMaterialEntranceItem::query();
        } else {
            $itemTable = 'raw_material_entrance_items';
            $entranceTable = 'raw_material_entrances';
            $entranceForeignKey = 'raw_material_entrance_id';
            $productType = ProductType::Raw->value;
            $query = RawMaterialEntranceItem::query();
        }

        return $query
            ->join($entranceTable . ' as entrances', 'entrances.id', '=', $itemTable . '.' . $entranceForeignKey)
            ->join('products', 'products.id', '=', $itemTable . '.product_id')
            ->join('suppliers', 'suppliers.id', '=', 'entrances.supplier_id')
            ->where('entrances.status', MaterialEntranceStatus::Closed->value)
            ->whereNotNull('entrances.closed_at')
            ->where('products.type', $productType)
            ->whereBetween('entrances.closed_at', [$startAt->toDateTimeString(), $endAt->toDateTimeString()])
            ->when(
                $supplierId > 0,
                fn ($query) => $query->where('entrances.supplier_id', $supplierId),
